...
row, applies the parameters to your parametric model, exports each variant as a SAT
body, and drops them all into a new design as separate components.
...
- **Build Variants Assembly from Sheet** — reads a Google Sheet, applies each row's
  parameters, exports SAT per variant, and assembles them into a new design.
...
`SheetVariants` folder inside this repo, then select it and click **Run**. Two buttons
appear under **Utilities → Add-Ins**.
...
4. Open your source model, run **Build Variants Assembly from Sheet**, paste the
   sheet link, set the component spacing, and run.

A new untitled design opens with one named component per variant.
...
A normal share/edit link is converted to a CSV-export link automatically; a
published-to-web CSV link is used as-is. Nothing is written back to the sheet.

## Limitations

- SAT carries solid geometry only — no parametric history. That's intended for a
